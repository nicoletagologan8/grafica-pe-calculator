...
void Display5() {
	...
	double a = 0.1;
	double b = 0.2;
	double tmax = 9.8; //bucata grafic
	xmax = fabs(a * (-tmax) - b * sin((-tmax)));
	ymax = fabs(a - b * cos((-tmax)));
	...
	for (double t = -tmax + ratia; t < tmax; t += ratia) {
		double x1, y1;
		x1 = (a * t - b * sin(t)) / (xmax * 1.05);
		y1 = (a - b * cos(t)) / (3.5 * ymax);
...
void Display7() {
	...
	for (double t = 0; t < tmax; t += ratia) {
		double x1, y1, r;
		r = a * exp(1 + t);
		x1 = r * cos(t) / (xmax * 1.05); //loc liber
		y1 = r * sin(t) / (2 * ymax);
